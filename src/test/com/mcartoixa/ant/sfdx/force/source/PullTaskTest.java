...
public class PullTaskTest {
    ...
    @Before
    public void setUp() {
        buildRule.configureProject("src/test/com/mcartoixa/ant/sfdx/force/source/pull.xml", Project.MSG_DEBUG);
    }
    ...
    @Test
    public void executeShouldAddWaitArgument() {
        buildRule.executeTarget("execute");
        Assert.assertTrue("Full log should contain -w argument", buildRule.getFullLog().contains("'-w60'"));
    }

    @Test
    public void executeShouldLogPulledSources() {
        buildRule.executeTarget("execute");
        Assert.assertTrue("Pulled sources should be logged", buildRule.getLog().contains("Changed ApexClass TestClass"));
    }
